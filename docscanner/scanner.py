...
import docscanner.transform as transform
import docscanner.imutils as imutils
...
class DocScanner(object):
    ...
    def scan(self, image_path):
        ...
        USER_ID = os.path.dirname(image_path).strip('/').split('/')[-1]
        SCAN_DIR = os.path.join(os.getcwd(), "scans", USER_ID)
        ...
        if os.path.exists(SCAN_DIR):
            cv2.imwrite(SCAN_DIR + '/' + basename, thresh)
    ...
    def create_document(self, image_dir):
        valid_formats = [".jpg", ".jpeg", ".jp2", ".png", ".bmp", ".tiff", ".tif"]

        get_ext = lambda f: os.path.splitext(f)[1].lower()

        image_files = [f for f in os.listdir(image_dir) if get_ext(f) in valid_formats]
        for img in image_files:
            self.scan(image_dir + '/' + img)
        
        USER_ID = image_dir.strip('/').split('/')[-1]
        SCAN_DIR = os.path.join(os.getcwd(), "scans", USER_ID)
        OUTPUT_DIR = os.path.join(os.getcwd(), "static", "content", USER_ID)
        if not os.path.exists(OUTPUT_DIR):
            os.makedirs(OUTPUT_DIR)

        # create a PDF from the scanned images
        pdf_path = os.path.join(OUTPUT_DIR, "document.pdf")
        imgs = [os.path.join(SCAN_DIR, img) for img in image_files]
        with open(pdf_path, "wb") as f:
            f.write(img2pdf.convert(imgs))
        
        pdf_rel_path = '/' + os.path.relpath(pdf_path, os.getcwd())
        print("Created PDF: " + pdf_rel_path)
